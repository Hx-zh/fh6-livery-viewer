...
## 一、数据源角色（2026-09-19 简化为 GameDB 系）

| 角色 | 数据源 | 提供什么 | 限制 |
|---|---|---|---|
| 唯一权威命名/年份源 | **游戏内存：GameDB 拍照文案**（`gamemem.read_car_years()`） | （年份, 完整车名）去重表，运行时解密 GameDB | 只认当前界面语言的文案格式（简中为「拍摄 YYYY 年份的 …」，界面语言须简中）；Traffic/NULL 等约 13 条无文案；条数随游戏状态浮动（658~671） |
| ID 锚点/新增探测 | **磁盘 `Data_Car.str`**（`<游戏>\media\Stripped\StringTables\EN.zip`） | 车型 ID（GameDB 文案不带 ID，ID 是涂装分组键——结构性必需）；DisplayName 短名仅作 join 键，**不作命名依据** | 短名非全名（无厂商前缀）；409 辆无年份；同实体可能残留多条（如 4221/4263）；后缀年份偶发与显示年份冲突（2163：后缀 '16，显示 2015）；**品牌展开表与后缀年份推导已废弃（2026-09-19，实证会错）** |
| 人工实证缓存 | cars.json 现存值 | 文案未覆盖条目维持原值（人工实证沉淀） | 无文案覆盖的新 ID 一律报「新增·待人工」，不猜名 |
| 旁证 | 官网 forza.net/fh6cars / 社区表 | 交叉校验 | 官方列表也有年份/命名错误（见 AGENTS.md），不作输入 |
...
| 脚本 | 用途 | 前提 |
|---|---|---|
| `tools/update_cars.py` | 磁盘 `.str`（ID 锚点）vs cars.json：ID 集合比对 + 内存拍照文案年份 join → diff 报告（默认模式；无文案覆盖的新 ID 报「新增·待人工」） | 只读游戏安装目录；内存数据源需游戏运行（界面语言简中） |
...
## 四、已知残留差异（历史记录，品牌展开表口径已废弃）

> 本节基于已废弃的 BR/FW/MM 品牌展开表口径——品牌展开表已随 2026-09-19 简化移除，本节历史结论已由 GameDB 拍照文案复核取代；重跑工具不再产生此类展开差异。

`.str` 展开规则（apply_all.js 的 BR/FW/MM 品牌表）与 cars.json 的差额，已全部逐条对照内存实证验证为 **cars.json 正确、品牌表口径旧**：
...
- （已完成，2026-09-19）数据源简化为 GameDB 系：品牌展开表与后缀年份推导移除（实证会错），无文案覆盖的新 ID 报「新增·待人工」
