...
| 「涂装筛选」 (Filters) | All filters are independent toggles that stack; duplicate analysis starts in the background the first time any toggle depending on it is switched on — zero cost otherwise. The duplicate judgment condition itself can be freely tuned in the「⚙ 重复检测参数」(duplicate-detection parameters) dialog. Includes the auction-only source toggle「仅显示拍卖涂装」(show only auction liveries) and the two auction-livery applied-status toggles —「仅显示已应用拍卖涂装(在车上)」/ show applied auction liveries (on car) and「仅显示未应用拍卖涂装(不在车上)」/ show unapplied auction liveries (not on car) — mutually exclusive with each other and with the applied/unapplied toggles; they run the same confirmation gate (scan on confirm, revert on cancel) if unscreened, and fall back to the `.manifest` registry when there is no memory snapshot (semantics = registered in the current cache generation, not the same as "on a car") |
...
├── lang_en.py / lang_ja.py / lang_ko.py / lang_zhtw.py  # translation tables (197 entries each)
