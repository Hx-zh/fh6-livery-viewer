...
| Card size (`[−] 100% [+]` / Ctrl+wheel) | Resize the card wall: the `[−] 100% [+]` control at the top-right of the toolbar (±10% per click), Ctrl+wheel over the wall, or the shortcuts Ctrl+= / Ctrl+- (numpad +/− also work) to zoom in/out and Ctrl+0 to reset to 100%; range 20%–200%, with thumbnails and text scaling proportionally. Session-only — back to 100% after a restart |
