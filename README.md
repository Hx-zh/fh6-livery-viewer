...
- 🔄 **重复涂装检测**：感知哈希 + 车型/作者/名称相似度多规则推断，区分四种场景——同车复刻（经典涂装多人复刻）/ 同车微调（v1/v2 版本）/ 跨车型移植 / 无图同名，可按场景筛选
...
| 操作 | 说明 |
|---|---|
| 单击卡片 | 选中，右侧显示大图与元数据（名称/车型/作者/游戏内位置/日期/层数/大小/重复组） |
| 双击卡片 | 打开可缩放预览窗口 |
| 右键卡片 | 弹出菜单：复制游戏内位置 / 名称 / 车型 / 作者 |
| Ctrl+C | 焦点在详情框时复制选中文本；否则复制选中涂装的游戏内位置 |
| 「重复涂装筛选」 | 按重复场景单选筛选（再点一次已选项取消），可叠加多/单涂装车型维度 |
| 「车厂」 | 按车厂过滤 |
